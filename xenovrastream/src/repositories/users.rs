...
impl<'d> UsersRepository<'d> {
    ...
    pub async fn create(&self, in_obj: InDBUser) -> XenovraStreamResult<User> {
        let id = Uuid::new_v4();
        ...
        .map_err(|e| match e {
            sqlx::Error::Database(dbe) if dbe.constraint() == Some("users_email_key") => {
                XenovraStreamError::AlreadyExists("user with given email".into())
            }
            _ => {
                tracing::error!("{e}");
    ...
    pub async fn get_by_email(&self, email: &str) -> XenovraStreamResult<User> {
