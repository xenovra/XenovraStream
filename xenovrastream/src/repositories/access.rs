...
impl<'d> AccessRepository<'d> {
    ...
    pub async fn create_or_update(
        ...
        .map_err(|e| match e {
            sqlx::Error::Database(ref dbe) if dbe.is_foreign_key_violation() => {
                XenovraStreamError::DoesNotExist(format!("storage with id \"{}\"", storage_id))
            }
            _ => {
                tracing::error!("{e}");
        ...
        if result.rows_affected() == 0 {
            return Err(XenovraStreamError::DoesNotExist(format!(
                "user with email \"{}\"",
    ...
    pub async fn delete_access(&self, user_id: Uuid, storage_id: Uuid) -> XenovraStreamResult<()> {
        sqlx::query(
            format!(
                "
            DELETE FROM {TABLE}
