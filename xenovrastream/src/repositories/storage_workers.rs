...
impl<'d> StorageWorkersRepository<'d> {
    ...
    pub async fn create(&self, in_obj: InStorageWorker) -> XenovraStreamResult<StorageWorker> {
        let id = Uuid::new_v4();
        ...
        .map_err(|e| match e {
            ...
            sqlx::Error::Database(dbe) if dbe.is_foreign_key_violation() => {
                XenovraStreamError::DoesNotExist("Such storage does not exist".to_string())
            }
            _ => {
                tracing::error!("{e}");
    ...
    pub async fn storage_has_any(&self, storage_id: Uuid) -> XenovraStreamResult<bool> {
    ...
    pub async fn list_by_user_id(&self, user_id: Uuid) -> XenovraStreamResult<Vec<StorageWorker>> {
        ...
        .fetch_all(self.db)
        .await
        .map_err(|_| XenovraStreamError::Unknown)
    }
    ...
    pub async fn get_token(
        ...
        storage_id: Uuid,
        limit: u8,
    ) -> XenovraStreamResult<Option<StorageWorkerTokenOnly>> {
