...
impl<'d> StoragesService<'d> {
    ...
    pub async fn create(
        ...
        if self
            ...
            .is_ok()
        {
            return Err(XenovraStreamError::StorageNameConflict);
        }
    ...
    pub async fn list(&self, user: &AuthUser) -> XenovraStreamResult<Vec<StorageWithInfo>> {
        self.repo.list_by_user_id(user.id).await
    }

    pub async fn get(&self, id: Uuid, user: &AuthUser) -> XenovraStreamResult<Storage> {
        check_access(&self.access_repo, user.id, id, &AccessType::R).await?;

        self.repo.get_by_id(id).await
    }

    pub async fn delete(&self, id: Uuid, user: &AuthUser) -> XenovraStreamResult<()> {
    ...
    pub async fn grant_access(
        ...
        if in_schema.user_email == user.email {
            return Err(XenovraStreamError::CannotManageAccessOfYourself);
        }
    ...
    pub async fn list_users_with_access(
        ...
        id: Uuid,
        user: &AuthUser,
    ) -> XenovraStreamResult<Vec<UserWithAccess>> {
        check_access(&self.access_repo, user.id, id, &AccessType::A).await?;
